...
import {BounceAnimator, ClickAnimationStyle} from './bounceAnimation.js';
...
const DockAbstractAppIcon = GObject.registerClass({
    ...
    _init(app, monitorIndex, iconAnimator) {
        ...
        // Initialize bounce animator
        const {settings} = Docking.DockManager;
        this._bounceAnimator = new BounceAnimator(settings);
    ...
    _onDestroy() {
        ...
        if (this._bounceAnimator) {
            this._bounceAnimator.destroy();
            delete this._bounceAnimator;
        }
    ...
    _triggerBounceAnimation() {
        // Debounce: prevent animation spam on multiple rapid clicks
        const now = Date.now();
        const minInterval = 100; // Minimum 100ms between bounces
        
        if (now - this._lastBounceTime < minInterval) {
            return; // Skip animation if clicked too soon
        }
        
        this._lastBounceTime = now;
        
        // Bounce only the icon image (iconBin), not the hover background
        if (this._bounceAnimator && this.icon && this.icon._iconBin) {
            try {
                this._bounceAnimator.bounceIcon(this.icon._iconBin, Docking.DockManager);
            } catch (e) {
                console.error('Dash to Dock 2: Error triggering bounce animation:', e);
            }
        }
    }
